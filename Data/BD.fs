...
module CinemaService =
    ...
    let createCinemaHall width height =
        let seats = Array2D.create height width { 
            Row = 0; 
            Column = 0; 
        ...
        { Width = width; Height = height; Seats = seats }
    ...
    let private listToSeats width height (seatList: Seat list) =
        let seats = Array2D.create height width { Row = 0; Column = 0; Status = SeatStatus.Available; BookedBy = None; BookingTime = None }
        seatList |> List.iteri (fun i seat ->
            let row = i / width
            let col = i % width
            seats.[row, col] <- seat)
        seats
    ...
    let saveCinemaData (cinema: CinemaHall) =
        try
            ...
            Result.Ok()
        with
        | ex -> Result.Error $"Failed to save data: {ex.Message}"
    ...
    let loadCinemaData () : Result<CinemaHall, string> =
        try
            if File.Exists(dataFilePath) then
                ...
                Result.Ok { Width = data.Width; Height = data.Height; Seats = seats }
            else
                ...
                Result.Ok cinema
        with
        | ex -> Result.Error $"Failed to load data: {ex.Message}"
    ...
    let bookSeat (cinema: CinemaHall) (request: BookingRequest) =
        if not (isValidSeat cinema request.Row request.Column) then
            ...
            | SeatStatus.Available ->
                ...
                | Result.Ok() -> Success $"Seat {request.Row}-{request.Column} successfully booked for {request.CustomerName}"
    ...
    // Cancel booking (make seat available)
    let cancelBooking (cinema: CinemaHall) row col =
        if not (isValidSeat cinema row col) then
            ...
            | SeatStatus.Booked ->
                ...
                match saveCinemaData cinema with
                | Result.Ok() -> Success $"Booking for seat {row}-{col} has been cancelled"
                | Result.Error msg -> Error msg
            | SeatStatus.Available ->
                Error "Seat is not currently booked"
            | _ ->
                Error "Invalid seat status"
    ...
    let getTotalSeatsCount (cinema: CinemaHall) =
        cinema.Width * cinema.Height
